...
void updatePlayer() {
	...
	// Fire bullets
	if (BUTTON_HELD(BUTTON_UP) && player.bulletTimer >= 20) {
...
void updateBall(BALL* b) {
	...
	if (b->active) {
		...
		for (int i = 0; i < BULLETCOUNT; i++) {
			if ((bullets[i].active) && (b->col, b->row, b->width, b->height, bullets[i].col, bullets[i].row,
